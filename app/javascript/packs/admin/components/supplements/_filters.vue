  ...
  <div class="filters">
    ...
    <div class='col-xs-12 well hidden'>
      <div class='form-group col-md-5'>
        <label>{{ $t('activerecord.models.supplement.name') }}</label>
        <input type="text" v-model="form.name_cont" class="form-control">
      </div>
      <div class='form-group col-md-5'>
        <label>{{ $t('activerecord.models.supplement.amount') }}</label>
        <select v-model="form.band_eq" class="form-control">
          <option v-for="supplement in supplements" :value="supplement.key">{{ supplement.name }}</option>
        </select>
      </div>
